...
interface VenueFollowFormProps {
  venueName: string
}

export default function VenueFollowForm({ venueName }: VenueFollowFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/venue-follow', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email.trim(), venue_name: venueName }),
      })
    ...
    return (
      ...
      >
        ...
        <p className="font-body font-light text-base" style={{ color: '#60A5FA' }}>
          We&apos;ll email you when new events are announced at {venueName}.
        </p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <p className="font-body font-light text-base mb-6" style={{ color: '#60A5FA' }}>
        Get an email the moment new events are announced at {venueName}.
      </p>
